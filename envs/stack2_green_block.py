...
class stack2_green_block(Base_Task):
    ...
    def load_actors(self):
        ...
        while (abs(green_pose.p[0]) < 0.05
               or np.sum(pow(green_pose.p[:2] - np.array([0, -0.1]), 2)) < 0.0225
               or np.sum(pow(green_pose.p[:2] - np.array([0, -0.13]), 2)) < 0.01):
            green_pose = rand_pose(
                xlim=[-0.28, 0.28],
                ylim=[-0.08, 0.05],
                zlim=[0.741 + block_half_size],
        ...
        self.green_block = create_block(green_pose, (0, 1, 0))
        self.blocks = [self.red_block, self.green_block]
