from ._base_task import Base_Task
from .utils import *
import sapien
import math


class stack_green_block(Base_Task):

    def setup_demo(self, **kwags):
        super()._init_task_env_(**kwags)

    def load_actors(self):
        block_half_size = 0.025
        center_pose = sapien.Pose([0, -0.13, 0.75 + self.table_z_bias], [0, 1, 0, 0])

        block_pose_lst = []
        for i in range(2):
            block_pose = rand_pose(
                xlim=[-0.28, 0.28],
                ylim=[-0.08, 0.05],
                zlim=[0.741 + block_half_size],
                qpos=[1, 0, 0, 0],
                ylim_prop=True,
                rotate_rand=True,
                rotate_lim=[0, 0, 0.75],
            )

            def check_block_pose(_block_pose):
                for j in range(len(block_pose_lst)):
                    if np.sum(pow(_block_pose.p[:2] - block_pose_lst[j].p[:2], 2)) < 0.01:
                        return False
                return True

            while (abs(block_pose.p[0]) < 0.05
                   or np.sum(pow(block_pose.p[:2] - np.array([0, -0.1]), 2)) < 0.0225
                   or np.sum(pow(block_pose.p[:2] - np.array([0, -0.13]), 2)) < 0.01
                   or not check_block_pose(block_pose)):
                block_pose = rand_pose(
                    xlim=[-0.28, 0.28],
                    ylim=[-0.08, 0.05],
                    zlim=[0.741 + block_half_size],
                    qpos=[1, 0, 0, 0],
                    ylim_prop=True,
                    rotate_rand=True,
                    rotate_lim=[0, 0, 0.75],
                )
            block_pose_lst.append(deepcopy(block_pose))

        def create_block(block_pose, color):
            return create_box(
                scene=self,
                pose=block_pose,
                half_size=(block_half_size, block_half_size, block_half_size),
                color=color,
                name="box",
            )

        self.red_block = create_block(center_pose, (1, 0, 0))
        non_red_colors = [((0, 1, 0), "green"), ((0, 0, 1), "blue")]
        color_ids = np.random.permutation(len(non_red_colors))
        non_red_ordered = [non_red_colors[i] for i in color_ids]

        block_a = create_block(block_pose_lst[0], non_red_ordered[0][0])
        block_b = create_block(block_pose_lst[1], non_red_ordered[1][0])
        self.blocks = [self.red_block, block_a, block_b]

        if non_red_ordered[0][1] == "green":
            self.green_block = block_a
        else:
            self.green_block = block_b

        for block in self.blocks:
            self.add_prohibit_area(block, padding=0.07)
        target_pose = [-0.04, -0.13, 0.04, -0.05]
        self.prohibited_area.append(target_pose)
        self.block1_target_pose = [0, -0.13, 0.75 + self.table_z_bias, 0, 1, 0, 0]

    def play_once(self):
        block_pose = self.green_block.get_pose().p
        arm_tag = ArmTag("left" if block_pose[0] < 0 else "right")

        self.move(self.grasp_actor(self.green_block, arm_tag=arm_tag, pre_grasp_dis=0.09))
        self.move(self.move_by_displacement(arm_tag=arm_tag, z=0.07))

        red_pose = self.red_block.get_pose().p
        target_pose = [red_pose[0], red_pose[1], red_pose[2] + 0.05, 0, 1, 0, 0]
        self.move(
            self.place_actor(
                self.green_block,
                target_pose=target_pose,
                arm_tag=arm_tag,
                functional_point_id=0,
                pre_dis=0.05,
                dis=0.,
                pre_dis_axis="fp",
            ))
        self.move(self.move_by_displacement(arm_tag=arm_tag, z=0.07))

        self.info["info"] = {
            "{A}": "red block",
            "{B}": "green block",
            "{a}": str(arm_tag),
        }
        return self.info

    def check_success(self):
        red_pose = self.red_block.get_pose().p
        green_pose = self.green_block.get_pose().p
        center_target = np.array([0.0, -0.13])
        center_eps = [0.03, 0.03]
        stack_eps = [0.025, 0.025, 0.012]

        red_on_center = np.all(abs(red_pose[:2] - center_target) < center_eps)
        green_on_red = np.all(abs(green_pose - np.array(red_pose[:2].tolist() + [red_pose[2] + 0.05])) < stack_eps)

        return (red_on_center and green_on_red
                and self.is_left_gripper_open() and self.is_right_gripper_open())
