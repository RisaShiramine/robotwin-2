...
class stack_trans_block(Base_Task):
    ...
    def _show_only_block(self, visible_block: Actor):
        blocks = [self.block1, self.block2, self.block3]
        for block in blocks:
            if block == visible_block:
                self._set_block_alpha(block, 1.0)
            else:
                self._set_block_alpha(block, 0.0)

    def play_once(self):
        self.last_gripper = None
        self.last_actor = None

        self._show_only_block(self.block1)
        arm_tag1 = self.pick_and_place_block(self.block1)

        self._show_only_block(self.block2)
        arm_tag2 = self.pick_and_place_block(self.block2)

        self._show_only_block(self.block3)
