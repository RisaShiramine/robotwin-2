...
class RoboticDiffusionTransformerModel(object):
    ...
    def load_pretrained_weights(self, pretrained=None):
        ...
        if filename.endswith(".pt"):
            ...
            # Handle both wrapped (checkpoint["module"]) and direct state_dict formats
            ...
            # load_file returns tensors in their saved dtype (may be float32).
            # Cast each tensor to match the corresponding model parameter's dtype
            # before calling load_state_dict to avoid dtype mismatch errors.
            state_dict = load_file(pretrained, device="cpu")
            model_state = self.policy.state_dict()
            casted = {}
            for k, v in state_dict.items():
                if k in model_state and v.dtype != model_state[k].dtype:
                    print(f"[load_pretrained] casting {k}: {v.dtype} -> {model_state[k].dtype}")
                    casted[k] = v.to(model_state[k].dtype)
                else:
                    casted[k] = v
            self.policy.load_state_dict(casted, strict=True)
        else:
    ...
    @torch.no_grad()
    def step(self, proprio, images, text_embeds, subgoal=None):
        """
        ...
        # ---- Fill arm plan dims 64-72 (MUST match training distribution) ----
        # Training always had state_indicator[64:73]=1 and arm plan encoded there.
        # If inference omits these, state_adaptor sees OOD mask=0 → model freezes.
        # Layout:
        #   dims 64-65: step0 arm [is_left, is_right]
        #   dims 66-67: step1 arm [is_left, is_right]
        #   dims 68-69: step2 arm [is_left, is_right]
        #   dims 70-72: current step one-hot [step==0, step==1, step==2]
        if subgoal is not None:
            arm_plan = subgoal.get("arm_plan", [2, 2, 2])   # 0=left 1=right 2=unk
            for s_idx in range(3):
                states[:, :, 64 + s_idx * 2]     = 1.0 if arm_plan[s_idx] == 0 else 0.0
                states[:, :, 64 + s_idx * 2 + 1] = 1.0 if arm_plan[s_idx] == 1 else 0.0
            step_s = int(subgoal["step"].item()) if hasattr(subgoal["step"], "item") else int(subgoal["step"])
            if 0 <= step_s <= 2:
                states[:, :, 70 + step_s] = 1.0
        # Always mark arm plan dims active to match training distribution
        state_elem_mask[:, 64:73] = 1.0
        # ------------------------------------------------------------------
