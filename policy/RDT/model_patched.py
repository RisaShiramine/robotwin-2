...
import models.rdt.blocks as blocks_module

def patched_forward(self, x: torch.Tensor, c: torch.Tensor, 
...
print("Patching CrossAttention.forward to capture attention weights...")
blocks_module.CrossAttention.forward = patched_forward
    ...
    while True:
        ...
        if len(ATTENTION_MAPS) > 0:
            try:
                ...
                output_dir = "eval_attention_maps"
                os.makedirs(output_dir, exist_ok=True)
                final_attn = ATTENTION_MAPS[-1]
                lang_len = lang_embeddings.shape[1] if len(lang_embeddings.shape) > 1 else lang_embeddings.shape[0]
                img_attn = final_attn[..., lang_len:]
                num_patches = 729
                front_img_attn = img_attn[..., 3*num_patches:4*num_patches]
                front_img_attn_mean = front_img_attn.mean(dim=(0, 1))
                raw_img = images[-3]
                if raw_img is not None:
                    vis_image = raw_img.resize((384, 384))
                    vis_image_np = np.array(vis_image)
                    grid_size = int(np.sqrt(num_patches))
                    plt.figure(figsize=(20, 5))
                    for k, step_idx in enumerate([0, 20, 40, 63]):
                        attn_map = front_img_attn_mean[3+step_idx].view(grid_size, grid_size).float().numpy()
                        if attn_map.max() - attn_map.min() > 0:
                            attn_map = (attn_map - attn_map.min()) / (attn_map.max() - attn_map.min())
                        attn_heatmap = cv2.resize(attn_map, (384, 384), interpolation=cv2.INTER_NEAREST)
                        heatmap = cv2.applyColorMap(np.uint8(255 * attn_heatmap), cv2.COLORMAP_JET)
                        overlay = cv2.addWeighted(vis_image_np, 0.5, heatmap, 0.5, 0)
                        plt.subplot(1, 4, k+1)
                        plt.imshow(overlay)
                        plt.title(f"Step {step_idx}")
                        plt.axis("off")
                    global GLOBAL_STEP_COUNTER
                    save_path = os.path.join(output_dir, f"step_{GLOBAL_STEP_COUNTER:04d}.png")
