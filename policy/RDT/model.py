...
def _apply_attn_patch():
    # Delayed import: called after make_policy so rdt.blocks is already loaded
    if _ATTN_PATCHED[0]:
        return
    try:
        import models.rdt.blocks as _blocks
        _blocks.CrossAttention.forward = _make_patched_cross_attn_forward()
        _ATTN_PATCHED[0] = True
        print('[Attn Vis] CrossAttention patched successfully.')
    except Exception as e:
        print(f'[Attn Vis] Patch failed: {e}')
# ===== END GLOBAL STATE =====
...
class RDT:
    ...
    def get_action(self, img_arr=None, state=None, subgoal=None):
        ...
        with torch.inference_mode():
            action_buffer = inference_fn(self.config, self.policy, self.lang_embeddings, self.observation_window, subgoal=subgoal).copy()
    ...
    def make_policy(self, args):
        ...
        if os.path.isfile(ckpt_config_path):
            ...
            # Keys that map directly into config_base_yaml["model"]
            _MODEL_KEYS = [
        ...
        _apply_attn_patch()  # patch CrossAttention after model/blocks are loaded
        return model


# RDT inference
def inference_fn(config, policy, lang_embeddings, observation_window, subgoal=None):
    ...
    while True:
        ...
        actions = (policy.step(proprio=proprio, images=images, text_embeds=lang_embeddings, subgoal=subgoal).squeeze(0).cpu().numpy())
